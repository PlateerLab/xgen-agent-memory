...
def learning_demo() -> None:
    ...
    st = mem.stats()
    print(
        f"learning: mean game rank {before:.1f} → {after:.1f} "
...
def distill_demo() -> None:
    mem = SynapseMemory(
        SynapseConfig(path=":memory:", epsilon=0.0, distill_epochs=12, distill_lr=5e-3)
    )
    ...
    m = mem.distill()
    print(
        f"distill: pairs={m.get('pairs'):.0f} swapped={m.get('swapped'):.0f} "
...
if __name__ == "__main__":
    for n in (500, 2000):
        ...
        bench_search(
            mem,
            ["리듬게임 판정", "김치찌개 레시피", "asyncio 디버깅", "오사카 여행"],
            label=f"{n} docs",
        )
        mem.close()
    learning_demo()
